    ...
    : m_request{}
{
    if (const auto *type_check_ptr = dynamic_cast<generic_request *>(request.get());
        !type_check_ptr)
    {
        throw server::request_parse_error("chechik_langberg_edges_handler: dynamic_cast failed!",
                                          request->type(), name());
    }
...
handle_return chechik_langberg_edges_handler::handle()
{
    ...
    chechik_langberg_edges_algorithm.setFaultSize(ft);

    if (std::string error; !chechik_langberg_edges_algorithm.preconditionsOk(ga, stretch, error))
    {
        throw std::runtime_error("Precondition for spanner not ok: " + error);
    }

    auto start = std::chrono::high_resolution_clock::now();
    chechik_langberg_edges_algorithm.call(ga, stretch, *spanner, in_spanner);
...
std::string chechik_langberg_edges_handler::name()
{
