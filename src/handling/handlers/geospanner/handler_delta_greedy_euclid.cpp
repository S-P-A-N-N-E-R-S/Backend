...
namespace server {
delta_greedy_euclid_handler::delta_greedy_euclid_handler(std::unique_ptr<abstract_request> request)
    : m_request{}
{
    if (const auto *type_check_ptr = dynamic_cast<generic_request *>(request.get());
        !type_check_ptr)
    {
        throw server::request_parse_error("delta_greedy_euclid_handler: dynamic_cast failed!",
                                          request->type(), name());
    }
...
handle_return delta_greedy_euclid_handler::handle()
{
    ...
    double delta = std::stod(m_request->graph_attributes().at("delta"));

    ogdf::EdgeArray<bool> in_spanner(graph);

    auto spanner = std::make_unique<ogdf::GraphCopySimple>(graph);

    ogdf::SpannerDeltaGreedyEuclidian delta_greedy_algorithm;

    //Scheduler enforces time limit
    delta_greedy_algorithm.setTimelimit(-1);
    delta_greedy_algorithm.setDelta(delta);

    if (std::string error; !delta_greedy_algorithm.preconditionsOk(ga, stretch, error))
    {
        throw std::runtime_error("Precondition for spanner not ok: " + error);
    }

    auto start = std::chrono::high_resolution_clock::now();
    delta_greedy_algorithm.call(ga, stretch, *spanner, in_spanner);
    ...
    return {std::unique_ptr<abstract_response>{new generic_response{
                &spanner_gm, &spanner_node_coords, &delta_greedy_algorithm.weights(), nullptr,
                nullptr, nullptr, nullptr, nullptr, nullptr, status_code::OK}},
            ogdf_time};
}

graphs::HandlerInformation delta_greedy_euclid_handler::handler_information()
{
    ...
    addFieldInformation(information, graphs::FieldInformation_FieldType_DOUBLE, "Parameter: delta",
                        "graphAttributes.delta", true);
...
std::string delta_greedy_euclid_handler::name()
{
    return "Delta Greedy (Euclidean)";
}
}  // namespace server