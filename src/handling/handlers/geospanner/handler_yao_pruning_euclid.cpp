...
namespace server {
yao_pruning_euclid_handler::yao_pruning_euclid_handler(std::unique_ptr<abstract_request> request)
    : m_request{}
{
    if (const auto *type_check_ptr = dynamic_cast<generic_request *>(request.get());
        !type_check_ptr)
    {
        throw server::request_parse_error("yao_pruning_euclid_handler: dynamic_cast failed!",
                                          request->type(), name());
    }
...
handle_return yao_pruning_euclid_handler::handle()
{
    ...
    double actual_stretch =
        yao_graph_algorithm.maxStretch(ga, spanner_yao, yao_graph_algorithm.weights());
    ...
    greedy.call(gy, stretch / actual_stretch, *spanner_final, in_spanner);
...
graphs::HandlerInformation yao_pruning_euclid_handler::handler_information()
{
...
std::string yao_pruning_euclid_handler::name()
{
    return "Yao Graph Pruning (Euclidean)";
}
}  // namespace server