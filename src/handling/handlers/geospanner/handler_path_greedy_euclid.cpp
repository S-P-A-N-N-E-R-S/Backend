...
namespace server {
path_greedy_euclid_handler::path_greedy_euclid_handler(std::unique_ptr<abstract_request> request)
    : m_request{}
{
    if (const auto *type_check_ptr = dynamic_cast<generic_request *>(request.get());
        !type_check_ptr)
    {
        throw server::request_parse_error("path_greedy_euclid_handler: dynamic_cast failed!",
                                          request->type(), name());
    }
...
handle_return path_greedy_euclid_handler::handle()
{
    ...
    ogdf::SpannerPathGreedyEuclidian path_greedy_algorithm;

    //Scheduler enforces time limit
    path_greedy_algorithm.setTimelimit(-1);

    if (std::string error; !path_greedy_algorithm.preconditionsOk(ga, stretch, error))
    {
        throw std::runtime_error("Precondition for spanner not ok: " + error);
    }

    auto start = std::chrono::high_resolution_clock::now();
    path_greedy_algorithm.call(ga, stretch, *spanner, in_spanner);
    ...
    return {std::unique_ptr<abstract_response>{new generic_response{
                &spanner_gm, &spanner_node_coords, &path_greedy_algorithm.weights(), nullptr,
                nullptr, nullptr, nullptr, nullptr, nullptr, status_code::OK}},
            ogdf_time};
}

graphs::HandlerInformation path_greedy_euclid_handler::handler_information()
{
...
std::string path_greedy_euclid_handler::name()
{
    return "Path Greedy (Euclidean)";
}
}  // namespace server