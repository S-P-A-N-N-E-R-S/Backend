...
/**
 * @brief Handler for the yao graph geospanner algorithm. Requests need as input a graph,
 ...
 * 
 */
class yao_graph_euclid_handler : public abstract_handler
{
public:
    yao_graph_euclid_handler(std::unique_ptr<abstract_request> request);

    virtual ~yao_graph_euclid_handler() = default;
